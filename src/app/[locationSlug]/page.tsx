                 ...
                 <div className="h-[30%] flex flex-col items-center justify-start pt-2 text-center px-5 w-full">
                   {step === 'direction' && (
                     <div className="absolute inset-0 z-[100] flex flex-col items-center overflow-hidden">
                        ...
                        <div className="absolute w-full px-6 flex flex-col items-center transition-all duration-1000 cubic-bezier(0.16, 1, 0.3, 1)" style={{ top: navPhase === 'verify' ? '41.5%' : '28.5%', transform: 'translateY(-50%)' }}>
                           <div className={`w-full max-w-sm animate-in fade-in zoom-in duration-1000 ${navPhase === 'verify' ? 'scale-90 opacity-40 blur-[1px]' : 'scale-100 opacity-100 blur-0'}`}>
                              <p className="text-[#000814] text-xl font-black tracking-tighter text-center" style={{ fontFamily: 'Barlow Semi Condensed' }}>{ (loc?.mapUrl && loc.mapUrl.includes('/') ? loc.mapUrl.split('/').pop()?.split('?')[0]?.replace(/%20/g, ' ') : '5F9Q+M5 LEIDEN') }</p>
                           </div>
                        </div>

                        {/* Map Container - Phase 1: Full | Phase 2: Hidden/Slide-down */}
                        <div className={`absolute left-0 right-0 px-5 transition-all duration-1000 cubic-bezier(0.16, 1, 0.3, 1) ${navPhase === 'maps' ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-[100%] pointer-events-none'}`} 
